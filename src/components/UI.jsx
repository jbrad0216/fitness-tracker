      ...
      return (
        <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
          ...
          <button
            ...
            className="px-6 h-14 bg-blue-500 text-white rounded-xl text-base font-semibold
          ...
          <button
            ...
            className="mt-3 text-base text-white/40 bg-transparent border-none cursor-pointer"
          >
            Reload App
          </button>
        </div>
...
export function Toast({ message, onDone }) {
  ...
  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50
      bg-green-500/90 text-white px-5 py-2.5 rounded-full text-base font-semibold
      ...
      {Array.from({ length: total }, (_, i) => {
        ...
        return (
          ...
          >
            ...
            <span className={`text-base mt-0.5 ${filled ? 'text-white' : 'text-white/30'}`}>
              32oz
            </span>
          </button>
        );
...
export function CardTitle({ children, right }) {
  return (
    <div className="flex justify-between items-center mb-2">
      ...
      {right && <span className="text-base text-white/50">{right}</span>}
    </div>
  );
}
  ...
  return (
    <input
      className={`bg-white/[0.06] border border-white/[0.08] rounded-[10px]
        px-3.5 py-3 text-white/90 text-lg outline-none w-full
...
export function Button({ children, variant = 'primary', className = '', onClick, ...props }) {
  ...
  return (
    <button
      className={`rounded-[10px] px-4 py-3 text-base font-semibold cursor-pointer
        transition-all duration-150 active:scale-95 active:opacity-80 border-none h-14
...
export function Label({ children }) {
  return (
    <label className="text-base text-white/50 uppercase tracking-wider mb-1 block">
      {children}
    </label>
  );
}
...
export function Toggle({ checked, onChange, label }) {
  return (
    <label className="flex items-center justify-between cursor-pointer select-none min-h-[48px]">
      {label && <span className="text-base text-white/80">{label}</span>}
...
export function OfflineIndicator() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[60] max-w-lg mx-auto
      bg-amber-500/95 text-white px-4 py-2 text-base font-semibold text-center
...
export function LandscapeHint() {
  return (
    <div className="landscape-hint hidden fixed inset-0 z-[9999] bg-[#0f1117]
      ...
      <p className="text-base text-white/60 text-center px-8">
...
export function PullToRefresh({ onRefresh, children }) {
  ...
  return (
    ...
    >
      {pullY > 10 && (
        <div
          className="flex justify-center items-center text-base text-white/40 overflow-hidden transition-all"
